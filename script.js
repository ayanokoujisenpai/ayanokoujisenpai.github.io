...
bgImage.onload = () => {
  ...
  document
    .querySelectorAll(".card, #home img, #services, #scammers")
    .forEach((el) => {
...
function handleSwipe() {
  ...
  const activeIndex = tabsArray.findIndex((tab) =>
...
const scammerData = [
  ...
  { name: "D. H.", number: "09931703051", source: "Me" },
  { name: "Unsure (D. H.)", number: "09855152830", source: "Ashley" },
];
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  searchInput.addEventListener("input", () => {
    ...
    const filtered = scammerData.filter(
      (entry) =>
